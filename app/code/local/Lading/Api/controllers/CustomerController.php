...
mail('shrutipatel@bacancytechnology.com','test','test');
	...
	public function updateAccountAction(){
		if (Mage::getSingleton ( 'customer/session' )->isLoggedIn()) {
	        ...
	        if (!empty($errors)) {
	            echo json_encode(array('code'=>1, 'message'=>$errors, 'data'=>array()));
	            return ;
	        }
	        try {
	        	$customer->save();
	        	echo json_encode(array('code'=>0, 'message'=>'success', 'data'=>$userData));
	        } catch (Mage_Core_Exception $e){
	        	echo json_encode(array('code'=>1, 'message'=>$e->getMessage(), 'data'=>array()));
	        } catch (Exception $e){
	        	echo json_encode(array('code'=>2, 'message'=>$e->getMessage(), 'data'=>array()));
	        }
	    } else {
	    	echo json_encode(array(
				'code' => 5,
	...
	public function updatePasswordAction()
	{
		if (Mage::getSingleton('customer/session')->isLoggedIn()) {
			...
			if($password && $new_password && ($validate == 0)){
				...
				try {
					$customer->save();
					echo json_encode(array('code' => 0, 'message' => 'success', 'data' => array()));
				} catch (Mage_Core_Exception $e) {
					echo json_encode(array('code' => 1, 'message' => $e->getMessage(), 'data' => array()));
				} catch (Exception $e) {
					echo json_encode(array('code' => 2, 'message' => $e->getMessage(), 'data' => array()));
				...
				echo json_encode(array('code' => 2, 'message' => 'password is not correct ', 'data' => array()));
			}
		}else {
			echo json_encode(array(
				'code' => 5,
