...
class Lading_Api_CartController extends Mage_Core_Controller_Front_Action {
	...
	public function addAction() {
		...
		if(Mage::getSingleton ( 'customer/session' )->isLoggedIn ()){
			try {
				...
				$result = array("code"=>0, "message"=> null, "data"=>array("items_qty"=>$items_qty));
				echo json_encode($result);
				return;
			} catch ( Exception $e ) {
				$result = array("code"=>1, "message"=>$e->getMessage () , "data"=>null);
				echo json_encode($result);
				return;
			}
		}else{
	...
	public function updateAction(){
		...
		try {
			//$cartData = $this->getRequest()->getParam();
	...
	public function useCouponAction() {
		...
		$messages['data'] = $this->_getCartTotal();
	...
	public function _getCartItems() {
		...
		foreach ( $items as $item ) {
			...
			$cartItemArr['available_qty'] = (int)Mage::getModel('cataloginventory/stock_item')->loadByProduct($item->getProduct ())->getQty();
			$cartItemArr ['is_available'] = (int)Mage::getModel('cataloginventory/stock_item')->loadByProduct($item->getProduct ())->getIsInStock();
